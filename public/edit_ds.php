...
if ($id < 0 || !($dsdetai->find($id))) { 
	redirect(BASE_URL_PATH); 
}

$errors = []; 
if ($_SERVER['REQUEST_METHOD'] === 'POST') { 
	if ($dsdetai->update($_POST)) { 
			...
			<!-- <main> -->
                ...
                <form name="frm" id="frm" action="" method="post" class="col-md-6 col-md-offset-3">
							...
							<div class="form-group<?= isset($errors['TenDeTai']) ? ' has-error' : '' ?>">
								...
								<textarea name="TenDeTai" id="TenDeTai" class="form-control" placeholder="Enter name (maximum character limit: 255)"><?= htmlspecialchars($dsdetai->TenDeTai) ?></textarea>
							...
							<div class="form-group<?= isset($errors['NoiDung']) ? ' has-error' : '' ?>">
								...
								<textarea name="NoiDung" id="NoiDung" class="form-control" placeholder="Enter project name (maximum character limit: 255)"><?= htmlspecialchars($dsdetai->NoiDung) ?></textarea>
							...
							<div class="form-group<?= isset($errors['GhiChu']) ? ' has-error' : '' ?>">
								...
								<textarea name="GhiChu" id="GhiChu" class="form-control" placeholder="Enter notes (maximum character limit: 255)"><?= htmlspecialchars($dsdetai->GhiChu) ?></textarea>
